...
| 2026-06-27 | **Stage 6 — ATS check (deterministic, client-side, no LLM → no Vercel timeout; resume never leaves the browser)**. New `src/lib/atsCheck.ts` `runAtsCheck(resumeMd, jdText)` → 0–100 score + Strong/Workable/At-risk rating + per-check items + matched/missing JD keywords. Five weighted checks: (1) JD keyword coverage 0.45 — curated word-bounded TECH list matched in JD then checked in resume (unmeasurable → neutral warn); (2) standard sections 0.20 (Experience/Skills/Education heading regex, Experience+Skills weighted); (3) parseable contact 0.15 (email essential, phone/link bonus); (4) ATS-safe formatting 0.12 (penalize markdown tables/images that linearize badly — .docx export drops them); (5) length sanity 0.08 (~200–1100 words). TECH list kept self-contained (NOT imported from `lib/server/jdExtract` — keeps the check client-only, no server code in the bundle). Wired into ResumeBuilder: auto-runs after Generate on `splitTailored(md).resumeMd`, renders a colour-toned panel (progress bar + per-item pass/warn/fail icons + "only add a keyword you can defend" honesty note) above the KB-suggestions panel; cleared when viewing a history entry (no JD context). Verified node: GOOD→100 Strong (8/8 kw), BAD→3 At-risk (no kw/sections/email + tables+images flagged), no-JD-kw→87 Strong w/ warn. Lint+build clean (no new deps; precache stays 20 entries). Resolves resume-analysis #1 ("real ATS check"). Browser-click test pending | ✅ |
| 2026-06-26 | **Stage 5 — render + ATS split (.docx + PDF)**, all client-side (resume never leaves browser). `src/lib/resumeRender.ts`: one Markdown block-parser → both renderers. `splitTailored()` strips appended Tailoring Inventory + Honesty notes so only the resume renders; `downloadDocx()` builds single-column ATS-safe Word via lazy `import('docx')`; `resumeToHtml()`+`printPdf()` = styled single-column print stylesheet → browser Save-as-PDF (vector, zero new render-runtime dep). Added `docx` dep; vite splits it into own `docx-writer` chunk + workbox globIgnore (precache stays 20 entries). ResumeBuilder: `.docx`+`PDF` buttons by Copy/.md, errors surfaced. Verified node: split separates resume/coaching, parseBlocks+html correct headings/bullets/bold, docx Packer→valid 8.7KB PK-zip. Resolves resume-analysis #1 (ATS .docx + designed PDF split). Browser-print test pending | ✅ |
| 2026-06-26 | **Stage 4 — tailor hardened (reasoning-enabled)**. llm.ts new `LLMOptions.thinking` flag (opt-in): gemini→dynamic `thinkingBudget:-1` (vs the 0 cap), anthropic→extended thinking block (budget=½ max_tokens) + robust text-block extraction, MiMo/OpenAI-compat reason intrinsically via token room. `api/resume/tailor.ts` runs `thinking:true`, maxTokens 4096, maxDuration 60. **System prompt rewritten from the 17 ChatGPT chats' real failures** (memory: resume-analysis + _ANALYSIS.md + [USER_CONSTRAINT] tags): expert ROLE (principal recruiter/strategist), explicit reason-first steps (JD priorities+exact phrasing→defensible evidence→ONE lane→lead/keep/drop), hard rules (truth-only: no invented Node/TS/ERPNext/fintech/metrics/years; one lane no spray; defensibility filter; tailoring≠bolding; problem/solution/domain bullets; concision not padding; no invented contact; ATS single-column), output appends **Tailoring Inventory + Honesty/Verification Notes**. Verified live (MiMo reasoning): nsave JD→Python lane, C++/Qt→1 line, mirrored 'RESTful APIs'/'PostgreSQL', honesty notes flagged AWS-not-in-CV/no-fintech/no-bg-processing/years nuance. Lint+build clean | ✅ |
